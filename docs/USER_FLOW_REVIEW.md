...
- Celery beat가 매 분 `process_next_pending_document`를 한 번 더 kick해, 최초 enqueue 실패나 self-chain 단절 시 다시 복구한다.
...
### 현재 적용 상태
- `documents`에 `failure_stage / failure_code / error_message` 컬럼을 추가했다.
- `upload_task`는 `preview`, `process` stage 실패를 DB와 task return payload에 함께 남긴다.
- `ProcessService.process_file()`는 `extract`, `classify`, `summarize` stage 실패를 분리 저장한다.
- summary 완료 후 index enqueue 실패는 문서 자체를 `FAILED`로 되돌리지 않고 별도 경고로 처리한다.
- 최초 enqueue 실패와 task self-chain 단절은 beat kick으로 다시 복구한다.
- `claim_next_pending_document()`는 conditional update 기반 claim으로 바꿔, 다중 worker에서 같은 문서를 중복 집는 가능성을 줄였다.
...
- session 문서는 `chat_session_references`와 `chat_session_reference_chunks`에 저장되고, retrieval 시 저장된 chunk를 우선 사용한다.
...
- 현재 확인된 품질/설계 이슈:
  - 세션 문서는 영속 chunk까지는 저장하지만, workspace/platform처럼 별도 vector/BM25 인덱스를 가지는 구조는 아니다.
- 구조적/확장 리스크:
  - 세션별 active task 추적은 여전히 Redis key `chat_task:{session_id}`를 기준으로 한다. enqueue 직전 경합은 `chat_task_lock:{session_id}`로 막지만, worker 내부에서의 더 강한 세션 직렬화까지 가진 구조는 아니다.

### 현재 적용 상태
- chat enqueue 실패는 `CHAT_ENQUEUE_FAILED`로 즉시 표면화한다.
- enqueue 실패 시 Redis `chat_task:{session_id}` 키는 남기지 않는다.
- enqueue 실패 시 방금 저장한 사용자 메시지와 세션 reference 변경은 보상 트랜잭션으로 되돌린다.
- 같은 세션에서 이미 답변 생성 중이거나 enqueue lock이 잡혀 있으면 `CHAT_ALREADY_PROCESSING`으로 즉시 차단한다.
- 프론트 `useChat`도 HTTP 전송 실패 시 optimistic 사용자 메시지와 참조 상태를 되돌린다.
- workspace retrieval query는 참조 의도를 드러내는 문장으로 보강한다.
- session retrieval query도 첨부 문서 참조 의도를 드러내는 문장으로 보강한다.
...
### 사용자 흐름
- 채팅 중 임시 문서 업로드
- reference 상태를 `processing / ready / failed`로 관리
- 비동기 추출/정규화
- 세션 reference 문서 저장
...
- Backend
  ...
  - `/api/chat/sessions/{session_id}/reference-upload`
  - `/api/chat/sessions/{session_id}/reference`

### 주요 상태 / 자원
- `chat_session_references`
- `chat_session_reference_chunks`
- `chat_session_references.extracted_text` (세션 첨부 원문)
- 세션 문서 추출 결과
- 세션 knowledge item

### 비동기 / 외부 연동
- ODL 추출
- Celery `chat_reference_queue`
- Ollama 답변 생성

### 검토 메모
- 세션 첨부 문서는 `chat_session_references` child entity로 분리했고, 업로드와 메시지 전송을 2단계 계약으로 나눴다.
- `reference-upload`는 temp file 저장 후 `chat_reference_queue`에 추출 task를 enqueue한다.
- `send_message(file_bytes=...)` legacy 경로는 제거했고, 채팅 계약은 `reference-upload` + `messages` 2단계로 통일했다.
- 세션 문서 payload는 `DocumentSchema`에서 `[본문] + [표]` 형태의 단일 텍스트로 조립한 뒤 reference row에 원문 그대로 저장한다.
- retrieval 시 session 지식원은 `chat_session_reference_chunks`를 우선 사용하고, chunk가 없을 때만 `chat_session_references.extracted_text`를 요청 시점에 paragraph 기반 chunk로 나눈다.
- answer context 조립 시 session block에는 `근거ID`와 `청크순번`을 같이 넣어 모델이 첨부 문서 근거를 식별할 수 있게 한다.
- answer context 조립 단계에서 session 블록은 top-k 제한만 적용한다. 별도 text trim은 하지 않는다.

### 발견 이슈
- 현재 확인된 품질/설계 이슈:
  - 세션 문서는 여전히 세션당 하나만 유지된다. 여러 첨부를 동시에 유지하는 모델은 아니다.
  - 세션 지식원은 chunk를 영속 저장하지만, workspace/platform처럼 별도 vector/BM25 인덱스를 갖는 구조는 아직 아니다.
- 구조적/확장 리스크:
  - 세션 문서는 별도 엔티티와 chunk를 갖게 되었지만, workspace/platform처럼 영속 검색 인프라까지 가진 것은 아니다. 따라서 “세션 문서만 재검색”, “세션 문서 citation”, “다중 첨부” 확장은 추가 설계가 필요하다.
  - 현재 구조는 세션 문서를 payload 컨텍스트로 취급하므로 workspace/platform처럼 동일한 retrieval 품질 계약을 적용할 수 없다.

### 현재 적용 상태
- 세션 문서는 단일 payload item 대신, 요청 시점에 paragraph 기반으로 chunking한 뒤 query lexical match로 상위 chunk를 고르는 방식으로 개선했다.
- session chunk는 `chunk_id`와 score를 갖고 retrieval 결과에 들어간다.
- 세션 문서는 저장 시 truncate를 제거했고, answer context 조립 시에도 별도 text trim을 하지 않는다.
...
- platform sync는 `platform_sync_queue`, maintenance 성격 작업은 `maintenance_queue`로 분리되어 있다.
...
- 구조적/확장 리스크:
  ...
  - platform sync와 maintenance는 분리됐지만, sync 자체는 여전히 source_type별 단일 실행 흐름이라 대규모 데이터에서 직렬 처리 시간이 길어질 수 있다.
...
1차 runtime contract는 적용을 시작했다.

- 공통 설계 문서:
  - `docs/ERROR_HANDLING_NORMALIZATION.md`
- 공통 모듈:
...
- `FailureStage` 도입
- 공통 failure payload builder 도입
- 문서 업로드 task 반환 payload 통일
- chat WS error payload에 `failure_stage/failure_code/error_message/retryable` 추가
- chat WS의 기존 `code/message`는 backward compatibility를 위해 유지
- export enqueue/zip build failure contract 적용
- platform sync enqueue/run failure contract 적용

아직 남은 범위:

- DB 레벨 `failure_*` 컬럼 도입 여부 결정
...
#### A. ChatSession의 group reference 필드

현재:
- `reference_group_id`

판단:
- 세션 첨부 문서는 이미 `chat_session_references`로 분리됐다.
- 현재 `ChatSession`에 남아 있는 것은 그룹 범위 선택을 위한 `reference_group_id`뿐이다.
- 그룹 참조를 별도 엔티티로 다시 분리할지 여부는 후순위 설계 항목이다.
