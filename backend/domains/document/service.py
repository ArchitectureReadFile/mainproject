...
from domains.document.summary_mapper import (
...
class DocumentService:
    ...
    def get_list(
        ...
        documents, total = self.repository.get_list(
            skip, limit, keyword, status, user_id, view_type, category, group_id
        )
        ...
        member_status_map = (
            self.repository.get_member_status_map(group_id=group_id, user_ids=owner_ids)
    ...
    def get_detail_in_group(
        ...
        member_status_map = self.repository.get_member_status_map(
            group_id=doc.group_id, user_ids=users_for_display
        )
        ...
        response_data = {
            ...
            "uploader": self._build_user_display_name(
                doc.owner, member_status_map.get(doc.owner.id) if doc.owner else None
            ),
            "summary_id": summary.id if summary else None,
            "title": self._build_document_title(doc, summary),
            "status": doc.processing_status.value,
            ...
            "assignee_username": self._build_user_display_name(
                assignee, member_status_map.get(assignee.id) if assignee else None
            ),
            ...
            "preview_status": doc.preview_status.value
    ...
    def get_unclassified_list(
        ...
        member_status_map = self.repository.get_member_status_map(
            group_id=group_id, user_ids=owner_ids
        )
    ...
    def get_preview_file_in_group(
        ...
        return preview_path, f"{download_name}.pdf"
    ...
    def delete_document(self, doc_id: int, user_id: int, group_id: int) -> None:
        ...
        if not is_uploader and doc.uploader_user_id:
            ...
            notif_service.create_notification_sync(
                user_id=doc.uploader_user_id,  # ✅ repository= 제거
