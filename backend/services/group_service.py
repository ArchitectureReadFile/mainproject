...
from models.model import MembershipRole, Group, GroupStatus, utc_now_naive
...
class GroupService:
    ...
    def invite_member(self, group_id: int, inviter_id: int, username: str, role: MembershipRole) -> InvitedMemberResponse:
        ...
        if existing:
            ...
            membership = self.repository.invite_member(target.id, group_id, inviter_id, role)
    ...
    def remove_member(self, target_id: int, group_id: int, remover_id: int) -> None:
        ...
        elif (target_membership.role == MembershipRole.ADMIN 
    ...
    # 멤버 권한 변경 
    def change_member_role(self, changer_id: int, target_id: int , group_id: int, role: MembershipRole) -> None:
        ...
        # 어드민은 어드민 변경x
        if changer.role == MembershipRole.ADMIN and target_membership.role == MembershipRole.ADMIN:
            raise AppException(ErrorCode.GROUP_NOT_OWNER)
