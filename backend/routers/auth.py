...
def login(
    ...
    user_resp, access_token, refresh_token = auth_service.login(db, redis, payload, client_ip)
...
def refresh(
    ...
    user_resp, access_token, refresh_token = auth_service.refresh(db, redis, refresh_token)
    CookieService.set_auth_cookies(response, access_token, refresh_token)
    return user_resp
