...
# UT-GRP-010-01 초대받은 사용자는 초대를 정상 거절할 수 있다.
@pytest.mark.parametrize("logged_in_user", [users[1]], indirect=True)
def test_decline_invite_success(client, db_session, logged_in_user):
    """초대받은 사용자는 워크스페이스 초대를 정상 거절하는지 검증한다."""
    ...
    res = client.post("/api/groups/1/members/decline")
...
# UT-GRP-010-02 초대받지 않은 사용자는 초대를 거절할 수 없다.
@pytest.mark.parametrize("logged_in_user", [users[1]], indirect=True)
def test_decline_invite_not_found_for_non_invited_user(
    client, db_session, logged_in_user
):
    """초대받지 않은 사용자는 워크스페이스 초대를 거절할 수 없는지 검증한다."""
    owner_data = users[0].copy()
    owner_data["password"] = auth_service.hash_password(owner_data["password"])
    owner = User(**owner_data)
    db_session.add(owner)
...
# UT-GRP-010-03 비활성 상태의 워크스페이스 초대는 거절할 수 있다.
@pytest.mark.parametrize("logged_in_user", [users[1]], indirect=True)
...
def test_decline_invite_success_for_non_active_group(
    client, db_session, logged_in_user, group_status, pending_reason
):
    """비활성 상태의 워크스페이스 초대는 거절할 수 있는지 검증한다."""
    owner_data = users[0].copy()
    owner_data["password"] = auth_service.hash_password(owner_data["password"])
    owner = User(**owner_data)
    db_session.add(owner)
...
# UT-GRP-010-04 비로그인 사용자는 초대를 거절할 수 없다.
def test_decline_invite_unauthenticated(client):
    """비로그인 사용자는 워크스페이스 초대 거절이 차단되는지 검증한다."""
    res = client.post("/api/groups/1/members/decline")
