...
def test_signup_failure_no_verification_A(client, fake_redis):
    payload = signup_data["payload"].copy()
...
def test_signup_failure_already_exists_B(client, registered_user, fake_redis):
    payload = signup_data["payload"].copy()
...
def test_signup_failure_username_exists_C(client, registered_user, fake_redis):
    payload = signup_data["payload"].copy()
...
def test_login_failure_invalid_credentials_A(client, registered_user, fake_redis):
...
def test_login_failure_rate_limit_C(client, registered_user, fake_redis):
    payload = login_data["payload"].copy()
    payload["email"] = registered_user.email
    client_key = f"testclient:{payload['email']}"
...
def test_refresh_token_failure_missing_cookie_A(client, fake_redis):
    response = client.post("/api/auth/refresh")
...
def test_confirm_account_failure_not_verified_A(client, fake_redis):
...
def test_confirm_account_failure_user_not_found_B(client, fake_redis):
...
def test_reset_password_failure_user_not_found_B(client, fake_redis):
