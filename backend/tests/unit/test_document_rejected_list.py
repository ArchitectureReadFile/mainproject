...
# UT-DOC-011-01 그룹 OWNER 또는 ADMIN은 본인이 반려한 문서 목록을 정상 조회할 수 있다.
...
def test_get_rejected_documents_success_for_owner_or_admin(
    client, db_session, logged_in_user, member_role
):
    """그룹 OWNER 또는 ADMIN은 본인이 반려한 문서 목록을 정상 조회하는지 검증한다."""
    now = utc_now_naive()
    ...
    db_session.add(
        Document(
            ...
            original_filename="rejected_doc_1.pdf",
            stored_path="/tmp/test_docs/rejected_doc_1.pdf",
    ...
    db_session.add(
        Document(
            ...
            original_filename="rejected_doc_2.pdf",
            stored_path="/tmp/test_docs/rejected_doc_2.pdf",
            processing_status=DocumentStatus.DONE,
            lifecycle_status=DocumentLifecycleStatus.ACTIVE,
        )
    )
    db_session.add(
        Document(
            ...
            group_id=GROUP_ID,
            uploader_user_id=uploader.id,
            original_filename="rejected_doc_other_reviewer.pdf",
            stored_path="/tmp/test_docs/rejected_doc_other_reviewer.pdf",
            processing_status=DocumentStatus.DONE,
            lifecycle_status=DocumentLifecycleStatus.ACTIVE,
        )
    )
    ...
    res = client.get(f"/api/groups/{GROUP_ID}/documents/rejected?skip=0&limit=10")
    ...
    assert data["total"] == 2
    assert set(ids) == {501, 502}


# UT-DOC-011-02 업로더 필터가 있는 경우 해당 업로더의 반려 문서만 조회할 수 있다.
@pytest.mark.parametrize("logged_in_user", [users[0]], indirect=True)
def test_get_rejected_documents_success_with_uploader_filter(
    client, db_session, logged_in_user
):
    """업로더 필터가 있는 경우 해당 업로더의 반려 문서만 조회하는지 검증한다."""
    now = utc_now_naive()
    ...
    db_session.add(
        Document(
            ...
            group_id=GROUP_ID,
            uploader_user_id=uploader.id,
            original_filename="filtered_rejected_doc.pdf",
            stored_path="/tmp/test_docs/filtered_rejected_doc.pdf",
            processing_status=DocumentStatus.DONE,
            lifecycle_status=DocumentLifecycleStatus.ACTIVE,
        )
    )
    db_session.add(
        Document(
            ...
            original_filename="other_rejected_doc.pdf",
            stored_path="/tmp/test_docs/other_rejected_doc.pdf",
            processing_status=DocumentStatus.DONE,
            lifecycle_status=DocumentLifecycleStatus.ACTIVE,
        )
    )
    ...
    res = client.get(
        f"/api/groups/{GROUP_ID}/documents/rejected?skip=0&limit=10&uploader={uploader.username}"
    )
    assert res.status_code == 200
...
# UT-DOC-011-03 OWNER 또는 ADMIN이 아닌 사용자는 반려 문서 목록을 조회할 수 없다.
@pytest.mark.parametrize("logged_in_user", [users[0]], indirect=True)
@pytest.mark.parametrize("member_role", [MembershipRole.EDITOR, MembershipRole.VIEWER])
def test_get_rejected_documents_forbidden_for_non_admin_or_owner(
    client, db_session, logged_in_user, member_role
):
    """OWNER 또는 ADMIN이 아닌 사용자는 반려 문서 목록을 조회할 수 없는지 검증한다."""
...
# UT-DOC-011-04 비로그인 사용자는 반려 문서 목록을 조회할 수 없다.
def test_get_rejected_documents_unauthenticated(client):
    """비로그인 사용자는 반려 문서 목록 조회가 차단되는지 검증한다."""
    res = client.get(f"/api/groups/{GROUP_ID}/documents/rejected?skip=0&limit=10")
