...
import { BellRing, FileUp, Inbox, Info, MessageSquare, ShieldAlert, Trash2, Users } from 'lucide-react'
      ...
      <div className="space-y-4">
        <div className="flex justify-end px-2">
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-bold text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800/50 cursor-pointer transition-all active:scale-95 focus:outline-none"
          >
            <Info size={14} />
            <span>도움말</span>
          </button>
        ...
        {showInfo && (
          <div className="bg-zinc-50 dark:bg-zinc-900/50 px-6 py-4 rounded-xl border border-zinc-100 dark:border-zinc-900 text-xs text-zinc-600 dark:text-zinc-400 space-y-2 animate-in fade-in slide-in-from-top-2 duration-200">
            <p><strong className="text-zinc-800 dark:text-zinc-200">목록 수신:</strong> 서비스 내 알림함(인박스)에 기록이 저장되어 언제든 다시 확인할 수 있습니다.</p>
            <p><strong className="text-zinc-800 dark:text-zinc-200">팝업 알림:</strong> 알림 발생 즉시 30초 동안 화면 우측 하단에 실시간 토스트 팝업으로 알려줍니다.</p>
          </div>
        )}
      </div>
                ...
                <div className="flex flex-col">
                  {category.items.map((item, index) => (
                    <div
                      key={item.type}
                      className={cn(
                        "flex flex-col md:flex-row md:items-center justify-between p-5 gap-4 hover:bg-zinc-50/50 dark:hover:bg-zinc-900/20 transition-colors",
                        index !== category.items.length - 1 && "border-b border-zinc-100 dark:border-zinc-800/50"
                      )}
                    >
                      <div className="flex gap-4 items-start">
                        <div className="w-8 h-8 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center shrink-0 mt-0.5">
                          <item.icon size={14} className="text-zinc-600 dark:text-zinc-400" />
                        </div>
                        <div>
                          <Label className="text-sm font-bold text-zinc-800 dark:text-zinc-200">{item.label}</Label>
                          <p className="text-[11px] font-medium text-zinc-400 dark:text-zinc-500 mt-0.5">{item.desc}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2 shrink-0">
                        <Button
                          ...
                          disabled={isUpdating}
                          className={cn(
                            "h-9 px-4 text-xs font-bold border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-900 active:scale-95 transition-all",
                            getStatus(item.type, 'is_enabled') ? "text-zinc-900 dark:text-zinc-100" : "text-zinc-400"
                          )}
                        >
                        ...
                        <Button
                          ...
                          className={cn(
                            "h-9 px-4 text-xs font-bold border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-900 active:scale-95 transition-all",
                            getStatus(item.type, 'is_toast_enabled') ? "text-zinc-900 dark:text-zinc-100" : "text-zinc-400"
                          )}
                        >
