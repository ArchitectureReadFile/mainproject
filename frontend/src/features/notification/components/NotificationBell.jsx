      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 max-h-[500px] bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-2xl z-50 overflow-hidden flex flex-col">
          <div className="p-3 border-b flex justify-between items-center bg-zinc-50/80 dark:bg-zinc-900/80 backdrop-blur-md">
            ...
            {unreadCount > 0 && (
              <Button variant="ghost" size="sm" onClick={markAllAsRead} className="text-[10px] h-7 font-bold hover:bg-zinc-200 dark:hover:bg-zinc-800">모두 읽음</Button>
            )}
          </div>

          <div className="flex px-1 border-b border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-950">
            <button onClick={() => setActiveTab('all')} className={cn("flex-1 py-2.5 text-[11px] transition-all", activeTab === 'all' ? "font-black text-zinc-900 dark:text-zinc-100 border-b-2 border-zinc-900 dark:border-zinc-100" : "text-zinc-400 hover:text-zinc-600")}>전체 ({notifications.length})</button>
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      !n.is_read ? "bg-blue-50/40 dark:bg-blue-900/15" : "hover:bg-zinc-50 dark:hover:bg-zinc-900/50",
                  ...
                  >
                    ...
                    <div className="flex gap-2.5 items-start">
                      ...
                      <div className="flex-1 pr-4">
                        <p className={cn("text-[11px] mb-1 leading-tight", !n.is_read ? "font-black text-zinc-900 dark:text-zinc-100" : "font-bold text-zinc-600 dark:text-zinc-400")}>{n.title}</p>
                        ...
                        <p className="text-[9px] text-zinc-400 dark:text-zinc-600 mt-2 font-medium">{n.displayTime}</p>
                      </div>
                    </div>

                    <button onClick={(e) => { e.stopPropagation(); deleteNotification(n.id); }} className="absolute top-4 right-2 p-1.5 text-zinc-300 hover:text-zinc-500 dark:hover:text-zinc-400 opacity-0 group-hover:opacity-100 transition-opacity" aria-label="삭제">
                      <X size={12} />
                    </button>
                  </div>
                )
